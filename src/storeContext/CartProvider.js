...
const CartProvider = props => {
  ...
  const reStore = async () => {
    try{
      ...
      const res = await axios.get(`https://crudcrud.com/api/c55dd8b9bc7840c3b5e077c25ba8ff77/cart${email}`);
  ...
  const removeCartItemHandler = async index => {
    ...
    try{
      const email = authCtx.userEmail.replace(/[@.]/g, "");
      const res = await axios.delete(`https://crudcrud.com/api/c55dd8b9bc7840c3b5e077c25ba8ff77/cart${email}/${backendId}`);
    } catch(error) {
